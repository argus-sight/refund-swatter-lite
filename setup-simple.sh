...
if [ "$SETUP_CRON" = "true" ]; then
    ...
    echo ""
    echo "  2. Click on 'process-notifications-cron' function"
    echo ""
    echo "  3. Go to 'Schedule' tab"
    echo ""
    echo "  4. Set cron expression: */5 * * * * (every 5 minutes)"
    echo ""
    echo "  5. Click 'Save'"
    echo ""
    echo -e "${GREEN}✓ Cron job can be configured via Dashboard${NC}"
fi
